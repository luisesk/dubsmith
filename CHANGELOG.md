...
- **The detected sync offset was written to a track that got discarded.** mkvmerge numbers tracks per input file and takes options as `<id>:<value>`; the code hardcoded `0` for `--sync`, `--language`, `--track-name` and `--default-track-flag`. The mkv mdnx produces has video at id 0 and audio at id 1, and `--no-video` drops that video right after, so on the `delay_ms >= 0` path the dub was muxed raw while the log reported the delay as applied. Negative delays were unaffected: they trim through ffmpeg first and the intermediate file is audio-only, where id 0 is correct. `_audio_track_id()` now reads the real id from `mkvmerge -J`. Files muxed before this fix with a positive delay carry an unsynced dub and need a remux; the track is already in the file, so `remux_with_new_delay` repairs them without re-downloading.
- **Tracks injected under an older `audio_label` were not recognized as ours.** Matching on the currently configured label alone let a track injected as `Portuguese` survive a remux under today's `Portuguese Brazil`, leaving the file with two tracks in the target language. `target_language.audio_label_aliases` lists labels the install used before; anything matching counts as a dubsmith injection. Empty by default. `remux_with_new_delay` also prefers our track when extracting instead of taking the first one in the target language, which may now be a preserved original.
